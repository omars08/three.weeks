...
Employee Employee_Records::top_gun_staff(int year)
{
    Employee top_gun;
    top_gun.set_salary(-1);
    int top_salary = 0;
    vector<Employee> empl_records = employee_repo.read_employee_files();
    //vector<Employee> year_records;
    //vector<Employee> highest_year_records;
    for(int i = 0; i < empl_records.size(); i++){
        if (empl_records[i].get_year() == year){
            if(empl_records[i].get_salary() >= top_gun.get_salary()){
                top_gun = empl_records[i];
            }
        }
    }

    if(top_gun.get_salary() == -1 ){
        cout << "No top gun found" << endl;
    }
